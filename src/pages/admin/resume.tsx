...
import { fetchResume,fetchAllResumes } from "@/redux/slice/resumeSlide";
import ViewDetailResume from "@/components/admin/resume/view.resume";
import { ALL_PERMISSIONS } from "@/config/permissions";
import Access from "@/components/share/access";
import { sfIn } from "spring-filter-query-builder";
import { EditOutlined } from "@ant-design/icons";
import { callFetchResumesByHRUserId } from "@/config/api";
            ...
            >
                <DataTable<IResume>
                    ...
                    request={async (params, sort, filter): Promise<any> => {
                        ...
                        // Dispatch với userId từ redux store
                        // dispatch(fetchResume({ userId: user.id }));
                        // Kiểm tra role để gọi API phù hợp
                        if (user.role?.name === "HR") {
                            await dispatch(fetchResume({ userId: user.id }));
                        } else {
                            const query = buildQuery(params, sort, filter);
                            await dispatch(fetchAllResumes({ query }));
                        }
